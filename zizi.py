    ...
    for i in files:
        with open(i,'r',encoding="utf-8",errors='ignore') as f:
            for j in f:
                if len(j.strip())!=0:
                    linecounter+=1
    tm.cprint('Lines of code:'+str(linecounter),'purple')
...
def Symbols_letters_Digits():
    ...
    for x in files:
      with open("oop-master",'r',encoding="utf-8",errors='ignore') as f: 
        for k in f:
            f.readline()
            ...
            for ch in k:
                   if ch.isalphabet():
                       ...
                       parser.update('digits',parser['digits']+1)
                   else:
                       parser.update('symbols',parser['symbols']+1)
...
def equality_statements():
    ...
    tm.cprint('\t If equality Statements found','purple')
    tm.cprint('=='*20,'yellow')
    id=1
    for k in fls:
        k=k.replace(' ','').replace('\t','')
        tm.cprint(str(id)+'.'+str(k),'blue')
        id+=1
    print('\n')
    tm.cprint('Elapsed Time for finding all equality statements :'+str(time()-starttime)+'\'s','yellow')

    equals=equality_statements()
    tm.cprint(' \t  If statments containing equality:','purple')
    tm.cprint('=='*20,'yellow')
    return len(fls)
